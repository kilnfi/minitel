...
import { TrxProtobuf } from './trx-protos';
...
export class TrxParser {
  public serializedToPb(tx_serialized: string): unknown {
    // FIXED: Use protobufjs directly instead of globalThis.proto
    const TransactionRaw = TrxProtobuf.lookupType('protocol.Transaction.raw');
    const txData = TransactionRaw.decode(Buffer.from(tx_serialized, 'hex'));
    const txDataObj = (txData as any).toObject();

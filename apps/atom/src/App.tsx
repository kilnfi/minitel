...
import { ATOM, PROTOCOLS, type Protocol } from '@protocols/shared';
import { Background, cn, Header, ProtocolTransactionDecoder } from '@protocols/ui';

const currentProtocol = ATOM;
      ...
      <div className="flex">
        <div className={cn('relative w-full transition-all duration-300 ease-in-out')}>
          ...
          <ProtocolTransactionDecoder
            adapter={createCosmosAdapter({ protocol: ATOM, name: 'atom', displayName: 'Cosmos' })}
          />
        </div>
      </div>
    </div>

...
import { OSMO, PROTOCOLS, type Protocol } from '@protocols/shared';
import { Background, cn, Header, ProtocolTransactionDecoder } from '@protocols/ui';

const currentProtocol = OSMO;
      ...
      <div className="flex">
        <div className={cn('relative w-full transition-all duration-300 ease-in-out')}>
          ...
          <ProtocolTransactionDecoder
            adapter={createCosmosAdapter({ protocol: OSMO, name: 'osmosis', displayName: 'Osmosis' })}
          />
        </div>
      </div>
    </div>
